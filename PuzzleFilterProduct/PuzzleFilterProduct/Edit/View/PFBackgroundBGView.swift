...
class PFBackgroundBGView: UIView {
    ...
    func setupSubView() {
        let leftIcon = UIImageView(image: UIImage(named: "icon_menu_alpha"))
        addSubview(leftIcon)
    ...
    func setupDataUI(){
        // 123456
        let model = PFBackgroundImageModel()
        for i in 0..<model.allBackImage.count {
            let x = i * 45
            let tempView = PFColorItemView(frame: CGRect(x: CGFloat(x), y: 0, width: 35, height: 35))
            scrollView1?.addSubview(tempView)
            let image = model.allBackImage[i]
            tempView.contentBtn.setBackgroundImage(image, for: .normal)
            tempView.tag = 200 + i
            tempView.clickBlock = {[weak self] (tag) in
                guard let sself = self, let image = model.allBackImage[tag - 200] else {
                    return
                }
                sself.clickSmContentBtn(tag, count: model.allBackImage.count)
                sself.colorSelBlock?(image)
            }
        }
        scrollView1?.contentSize = CGSize(width: CGFloat(model.allBackImage.count * 45), height: 35)
    ...
    func clickSmContentBtn(_ tag: Int, count: Int) {
        for i in 0..<count {
            let btn = scrollView1?.viewWithTag(200 + i) as! PFColorItemView
            if tag == 200 + i {
                btn.isSelect = true
            } else {
                btn.isSelect = false
